...
if (command === "test") {
  ...
  try {
    const mod = await import(resolve("prov.config.ts"));
    config = mod.default ?? {};
  } catch { /* no config */ }
  ...
  console.log("  prov hierarchy --platform <p> Dump element hierarchy as JSON");
  console.log("  prov selectors --platform <p> List actionable elements with suggested selectors");
  console.log("  prov validate [path]          Validate flow files without device connection");
  console.log("  prov devices                  List connected devices across platforms");
