/**
 * iOS raw driver implemented on top of the generic Appium client.
 *
 * All operations go through Appium HTTP endpoints --- no xcrun, no iproxy,
...
export function createAppiumIOSDriver(
  ...
  const service: RawDriverService = {
    ...
    // -----------------------------------------------------------------------
    // Coordinate-level actions (W3C touch pointer actions for iOS)
    // -----------------------------------------------------------------------
    tapAtCoordinate: (x, y) =>
      Effect.tryPromise({
        try: () =>
      ...
      Effect.tryPromise({
        try: async () => {
          ...
          return {
            ...
            name: (caps["deviceName"] as string) ?? "iOS Device (Appium)",
            isEmulator: false,
      ...
      Effect.tryPromise({
        try: async () => {
          ...
          if (opts?.clearKeychain) {
            console.warn(
              "clearKeychain is not available via Appium on iOS. " +
                "Use local mode with xcrun simctl for keychain operations.",
            );
          }
    ...
    clearAppState: (bundleId) =>
      Effect.tryPromise({
        try: async () => {
          ...
          new DriverError({
            message:
              `Clear app state failed: ${e}. ` +
              "Note: clearApp may not be available on all iOS Appium providers. " +
              "If this persists, consider using terminate + uninstall + install instead.",
          }),
      }),
    ...
    back: () =>
      Effect.fail(
        new DriverError({
          message:
            "back() is not supported on iOS. Use navigation within the app or swipe gestures.",
        }),
      ),
